    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('sensores.store') }}" class="space-y-4">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <x-text-input id="valor_minimo" name="valor_minimo" type="number" step="0.01" class="mt-1 block w-full" :value="old('valor_minimo')" />
                            <x-input-error :messages="$errors->get('valor_minimo')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="valor_maximo" value="Valor Máximo" />
                            <x-text-input id="valor_maximo" name="valor_maximo" type="number" step="0.01" class="mt-1 block w-full" :value="old('valor_maximo')" />
                            <x-input-error :messages="$errors->get('valor_maximo')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="unidad_medida" value="Unidad" />
                            <x-text-input id="unidad_medida" name="unidad_medida" type="text" class="mt-1 block w-full" :value="old('unidad_medida')" placeholder="°C, A, bar..." />
                    ...
                    <div>
                        ...
                        <x-text-input id="precision_sensor" name="precision_sensor" type="number" step="0.01" class="mt-1 block w-full" :value="old('precision_sensor')" />
                    ...
                    <div>
                        ...
                        <select id="dispositivos_iot_id" name="dispositivos_iot_id" class="mt-1 block w-full border-gray-300 rounded-md" required>
                            ...
                            @foreach ($dispositivos as $dispositivo)
                                <option value="{{ $dispositivo->id }}" @selected((string) old('dispositivos_iot_id') === (string) $dispositivo->id)>
                    ...
                    <div class="flex items-center gap-4">
                        <x-primary-button>Guardar Sensor</x-primary-button>
