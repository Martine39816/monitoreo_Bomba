    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('mantenimientos.update', $mantenimiento->id) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select id="tipo" name="tipo" class="mt-1 block w-full border-gray-300 rounded-md" required>
                            @foreach ($tipos as $tipo)
                                <option value="{{ $tipo }}" @selected(old('tipo', $mantenimiento->tipo) === $tipo)>
                                    {{ ucfirst($tipo) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="bombas_id" name="bombas_id" class="mt-1 block w-full border-gray-300 rounded-md" required>
                            @foreach ($bombas as $bomba)
                                <option value="{{ $bomba->id }}" @selected((string) old('bombas_id', $mantenimiento->bombas_id) === (string) $bomba->id)>
                        ...
                        <select id="alertas_id" name="alertas_id" class="mt-1 block w-full border-gray-300 rounded-md">
                            ...
                            @foreach ($alertasAbiertas as $alerta)
                                <option value="{{ $alerta->id }}"
                                    @selected((string) old('alertas_id', $mantenimiento->alertas_id) === (string) $alerta->id)>
                    ...
                    <div>
                        ...
                        <x-text-input id="fecha" name="fecha" type="date" class="mt-1 block w-full" :value="old('fecha', $mantenimiento->fecha->format('Y-m-d'))" required />
                        <x-input-error :messages="$errors->get('fecha')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="descripcion" value="Descripción del trabajo realizado" />
                        <textarea id="descripcion" name="descripcion" rows="4" class="mt-1 block w-full border-gray-300 rounded-md">{{ old('descripcion', $mantenimiento->descripcion) }}</textarea>
                        <x-input-error :messages="$errors->get('descripcion')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="observaciones" value="Observaciones" />
                        <textarea id="observaciones" name="observaciones" rows="2" class="mt-1 block w-full border-gray-300 rounded-md">{{ old('observaciones', $mantenimiento->observaciones) }}</textarea>
                        <x-input-error :messages="$errors->get('observaciones')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="proximo_mantenimiento" value="Próximo Mantenimiento (opcional)" />
                        <x-text-input id="proximo_mantenimiento" name="proximo_mantenimiento" type="date" class="mt-1 block w-full" :value="old('proximo_mantenimiento', $mantenimiento->proximo_mantenimiento?->format('Y-m-d'))" />
